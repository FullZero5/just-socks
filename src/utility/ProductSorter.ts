...
export type SortBy = 'price' | 'category' | 'color' | 'size';
export type SortOrder = 'asc' | 'desc';

const sortFunctions: { [key in SortBy]: (a: Product, b: Product, order: SortOrder) => number | undefined } = {
  price: (a, b, order) => order === 'asc' ? a.price - b.price : b.price - a.price,
  category: (a, b, order) => order === 'asc' ? a.category.localeCompare(b.category) : b.category.localeCompare(a.category),
  color: (a, b, order) => order === 'asc' ? a.color.localeCompare(b.color) : b.color.localeCompare(a.color),
  size: (a, b, order) => {
...
export class ProductSorter {
  ...
  static sortProducts(
    ...
    if (size !== undefined) {
      filteredProducts = products.filter(product => product.size?.includes(size));
    }
